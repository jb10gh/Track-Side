...
import { EVENT_TYPES, TEAMS } from '../../store/gameStoreClean';

// Memoize static action cards to prevent recreation
const ACTION_CARDS = [
  {
    id: 'goal-us',
...
const ActionCard = memo(({ 
  ...
  return (
    <motion.div
      ...
      animate={isSwipingUp && position === 'center' ? 'swipeUp' : position}
      style={{ zIndex: position === 'center' ? 10 : 5 }}
    >
      <div
        className={`
...
export const GestureDeckOptimized = memo(({ onAction }) => {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [isExpanded, setIsExpanded] = useState(false);
  const [swipeDirection, setSwipeDirection] = useState(null);
  const deckRef = useRef(null);

  // Memoize current card to prevent unnecessary recalculations
  const currentCard = useMemo(() => ACTION_CARDS[currentIndex], [currentIndex]);

  // Memoize gesture handlers to prevent recreation
  ...
  const handleSwipeLeft = useCallback(() => {
    ...
    setTimeout(() => {
      setCurrentIndex((prev) => (prev + 1) % ACTION_CARDS.length);
  ...
  const handleSwipeRight = useCallback(() => {
    ...
    setTimeout(() => {
      setCurrentIndex((prev) => (prev - 1 + ACTION_CARDS.length) % ACTION_CARDS.length);
      setSwipeDirection(null);
    }, 200);
  }, []);
  ...
  // Memoize card positions to prevent recalculations
  const cardPositions = useMemo(() => {
    const positions = {};
    
    ACTION_CARDS.forEach((_, index) => {
      if (index === currentIndex) {
        positions[index] = 'center';
      } else if (index === (currentIndex - 1 + ACTION_CARDS.length) % ACTION_CARDS.length) {
        positions[index] = 'left';
      } else if (index === (currentIndex + 1) % ACTION_CARDS.length) {
...
GestureDeckOptimized.displayName = 'GestureDeckOptimized';
