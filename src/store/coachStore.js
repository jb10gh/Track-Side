...
const generateActionItems = (stats) => {
    ...
    if (stats.goalDifference > 0) {
        actionItems.push('✅ Strong offensive performance - maintain momentum');
    } else if (stats.goalDifference < 0) {
        actionItems.push('🎯 Focus on defensive organization in next training');
    }
    
    if (stats.ourPenalties > stats.theirPenalties) {
        actionItems.push('⚠️ Review discipline - reduce penalty count');
    }
    
    if (stats.ourGoals === 0) {
        actionItems.push('🔥 Work on finishing and scoring opportunities');
    }
    
    if (actionItems.length === 0) {
        actionItems.push('👍 Solid performance - continue current strategy');
    }
