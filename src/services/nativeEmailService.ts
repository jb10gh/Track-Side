...
export class NativeEmailService {
  ...
  private generateBeautifulBody(matchData: MatchData): string {
    ...
    const summary = [
      `📊 MATCH SUMMARY`,
      `═══════════════════════════`,
      `🏆 Match: Our Team vs ${matchData.opponentName}`,
      `⚡ Our Team: ${matchData.myScore}, Opponent: ${matchData.opponentScore}`,
      `📅 Date: ${new Date(matchData.timestamp).toLocaleDateString()}`,
      `⏱️ Duration: ${matchData.finalTime || 'Unknown'}`,
      ...
      '📊 TEAM STATISTICS',
      '═══════════════════════════',
      `🔵 Our Team: ${matchData.myScore} goals`,
      `🔴 ${matchData.opponentName}: ${matchData.opponentScore} goals`,
      '',
      `🌐 ${productionUrl}`,
      '',
